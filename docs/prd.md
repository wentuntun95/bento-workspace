...
**表情包**：来源 [diyCutie](https://github.com/Diudiu-wl/diyCutie)，感谢原作者 ❤️ 12种情绪，可自行替换

**注意**：手机端语音对话暂不可用，桌面端正常。
- **原因**：Web Speech API 在部分移动浏览器（尤其 iOS Safari / PWA 模式）权限受限，无法稳定触发
- **解决方向**：改用服务端 ASR（如 MiniMax 语音转文字接口），或引导用户使用键盘输入作为备选
